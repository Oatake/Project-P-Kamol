    ...
    for x in range(4*3):
        for y in range(4):
            if (x%3==0) :
                    ...
                    # print(lk)
                    # print(dm.getLocker(lk)["status"])
                    # tkinter.Button(frame, image=dt_img["busy"], command=partial(ShowLocker,row=y,column=x)).grid(row=x, column=y)
                try :
                    lk="Locker"+str(x)+str(y)
                    # print(dm.getLocker(lk)["status"])
...
locker_frame = create_locker_frame(GUI,btn_matrix)
locker_frame.grid(row=0,column=0,columnspan=3)  
